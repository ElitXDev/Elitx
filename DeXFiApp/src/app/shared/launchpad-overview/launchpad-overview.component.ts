...
@Component({
  selector: 'app-launchpad-overview',
  templateUrl: './launchpad-overview.component.html',
  styleUrls: ['./launchpad-overview.component.scss'],
})
export class LaunchpadOverviewComponent implements OnInit {
  ...
  async setAvailableNfts() {
    try {
      var hash: string = shajs('sha256')
        ...
        .digest('hex')
        .toUpperCase();
      for (const [i, _] of this.projects.entries()) {
        var state = (await this.x.client.request({
          command: 'account_namespace',
          account: this.projects[i].address,
          namespace_id: hash,
        })) as AccountState;
        // console.log('Hook state:', state);
        for (const entry of state.result.namespace_entries) {
          if (parseInt(entry.HookStateKey[63], 16) == 9) {
            for (const [j, _] of this.projects[i].shares.entries()) {
              var idx = (this.projects[i].shares.length + j) * 2;
              this.projects[i].shares[j] =
                this.projects[i].maxShares[j] -
